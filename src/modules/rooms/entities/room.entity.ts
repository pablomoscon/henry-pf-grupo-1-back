...
export class CreateRoomDto {
    @ApiProperty({ description: 'Name of the room', example: 'Deluxe Suite' })
    @IsString()
    name: string;
    ...
    @ApiProperty({ description: 'Price of the room', example: 150.00 })
    @IsNumber()
    price: number;

    @ApiProperty({ description: 'Availability of the room', example: true })
    @IsBoolean()
    available: boolean;
}